...
import org.easydarwin.video.BuildConfig;
...
public class HWConsumer extends Thread implements VideoConsumer {
    ...
    private EasyPlayerClient2 easyPlayerClient2;

    final int millisPerFrame = 1000 / 30;
    long lastPush = 0;

    public HWConsumer(Context context, String mime, Pusher pusher, int bitrateKbps, String mName, int mColorFormat, EasyPlayerClient2 client) {
        mContext = context;
        mPusher = pusher;
        mMime = mime;
                ...
                try {
                    if (mPusher != null) {
                        ...
                        if (sync) {
                            ...
                            length = mPpsSps.length + bufferInfo.size;
                            mPusher.push(h264, 0, length, bufferInfo.presentationTimeUs / 1000, 2);
                            ...
                            mPusher.push(h264, 0, length, bufferInfo.presentationTimeUs / 1000, 1);
                        ...
                        if (easyPlayerClient2.isRecording()) {
                            ...
                            frameInfo.buffer = h264;
                            frameInfo.offset = 0;
                            frameInfo.length = length;

                            easyPlayerClient2.pumpVideoSample(frameInfo);
                        }
                    }
                } catch (IOException e) {
                    e.printStackTrace();
